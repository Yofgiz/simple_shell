...
int builtin_exit(program_data *data)
{
	...
	{/*if exists arg for exit, check if is a number*/
		for (i = 0; data->tokenized_input[1][i]; i++)
			if ((data->tokenized_input[1][i] < '0' || data->tokenized_input[1][i] > '9')
				&& data->tokenized_input[1][i] != '+')
			{/*if is not a number*/
				errno = 2;
				return (2);
			}
		errno = _atoi(data->tokenized_input[1]);
	}
	free_all_data(data);
	exit(errno);
}
...
int builtin_cd(program_data *data)
{
	...
	if (data->tokenized_input[1])
	{
		if (str_compare(data->tokenized_input[1], "-", 0))
		{
		...
		{
			return (set_work_directory(data, data->tokenized_input[1]));
		}
	}
	else
	{
...
int builtin_help(program_data *data)
{
	...
	{
		length = str_length(data->tokenized_input[1]);
		if (str_compare(data->tokenized_input[1], mensajes[i], length))
		{
...
int builtin_alias(program_data *data)
{
	...
	{/* if there are arguments, set or print each env variable*/
		if (count_characters(data->tokenized_input[i], "="))
			set_alias(data->tokenized_input[i], data);
		else
			print_alias(data, data->tokenized_input[i]);
	}
